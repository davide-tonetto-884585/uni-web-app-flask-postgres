...
def get_presenze_lezione(user, id_corso, id_prog, id_lezione):
    ...
    presenze = sessionDocenti.query(Utente.id, Utente.nome, Utente.cognome).filter(
        Utente.id == PresenzeLezione.id_studente, PresenzeLezione.id_programmazione_lezioni == id_lezione)
...
def add_iscrizione(user, id_corso, id_prog):
    ...
    if 'studente' in user.roles and user.id != request.form.get('id_studente'):
        return jsonify({'error': True, 'errormessage': 'Permission denied'}), 404
